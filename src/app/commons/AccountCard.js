...
const ColoredStatus = ({online}) => {
    return (
        <div class="h-2 w-2 rounded-full mr-2 inline-block" style={{background: online ?statusToColor['Online']: 'gray'}}></div>
    )
}


const columns = [{name: 'label'}, {name: 'address'},{name: 'network', render: (value)=><><ColoredNetwork network={value} /><span>{value}</span></>},{name: 'value'}, {name: 'online', render: (value)=><ColoredStatus online={value}/>}]
